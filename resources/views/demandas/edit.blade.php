                    ...
                    <form method="POST" action="{{ route('demandas.update', $demanda) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="relative">
                                    <input type="hidden" id="condominio_id" name="condominio_id" value="{{ old('condominio_id', $demanda->condominio_id) }}" required>
                                    <input type="text" 
                                           ...
                                           autocomplete="off"
                                           value="{{ old('condominio_search', $demanda->condominio->nome) }}">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="titulo" class="block mt-1 w-full" type="text" name="titulo" :value="old('titulo', $demanda->titulo)" required autofocus />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="descricao" name="descricao" rows="5" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>{{ old('descricao', $demanda->descricao) }}</textarea>
                            ...
                            <div>
                                ...
                                <select id="urgencia" name="urgencia" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    <option value="baixa" {{ old('urgencia', $demanda->urgencia) == 'baixa' ? 'selected' : '' }}>Baixa</option>
                                    <option value="media" {{ old('urgencia', $demanda->urgencia) == 'media' ? 'selected' : '' }}>Média</option>
                                    <option value="alta" {{ old('urgencia', $demanda->urgencia) == 'alta' ? 'selected' : '' }}>Alta</option>
                                    <option value="critica" {{ old('urgencia', $demanda->urgencia) == 'critica' ? 'selected' : '' }}>Crítica</option>
                                </select>
                            ...
                            <div class="md:col-span-2">
                                <x-input-label :value="__('Fotos Anexadas')" />
                                <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
                                    @foreach($demanda->anexos as $anexo)
                                        <div class="relative">
                                            @if(str_starts_with($anexo->mime_type, 'image/'))
                                                <img src="{{ Storage::url($anexo->caminho) }}" alt="{{ $anexo->nome_original }}" class="w-full h-24 object-cover rounded border border-gray-300">
                                            @else
                                                <div class="w-full h-24 bg-gray-200 rounded border border-gray-300 flex items-center justify-center">
                                                    <span class="text-xs text-gray-600">{{ $anexo->nome_original }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    @endforeach
                            ...
                            <!-- Novos Anexos (Fotos) -->
                            <div class="md:col-span-2">
                                <x-input-label for="anexos" :value="__('Adicionar Mais Fotos (opcional)')" />
                                <input type="file" 
                                       id="anexos" 
                                       name="anexos[]" 
                                       multiple 
                                ...
                                <div id="prestadores-container" class="mt-2 space-y-2 max-h-80 overflow-y-auto border border-gray-300 dark:border-gray-700 rounded p-3">
                                    @foreach($prestadores as $prestador)
                                        <label class="prestador-item flex items-start hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded cursor-pointer border border-transparent hover:border-gray-300 dark:hover:border-gray-600" 
                                            ...
                                            <input type="checkbox" name="prestadores[]" value="{{ $prestador->id }}" class="mt-1 rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array($prestador->id, old('prestadores', $prestadoresSelecionados)) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('demandas.show', $demanda) }}" class="mr-4 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">
                                Cancelar
                            </a>
                            <x-primary-button>
                                {{ __('Atualizar Demanda') }}
                            </x-primary-button>
                        </div>
                    </form>
                </div>
