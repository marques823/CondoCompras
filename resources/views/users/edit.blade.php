        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('users.update', $userModel->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $userModel->name)" required autofocus />
                            ...
                            <div>
                                ...
                                <select id="perfil" name="perfil" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($perfis as $key => $label)
                                        <option value="{{ $key }}" {{ old('perfil', $userModel->perfil) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div id="email-field">
                                ...
                                <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $userModel->email)" />
                            ...
                            <div id="telefone-field">
                                ...
                                <x-text-input id="telefone" class="block mt-1 w-full" type="tel" name="telefone" :value="old('telefone', $userModel->telefone)" placeholder="(00) 00000-0000" />
                            ...
                            <div id="empresa-select-field">
                                ...
                                <select id="administradora_id" name="administradora_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach($administradoras as $empresa)
                                        <option value="{{ $empresa->id }}" {{ old('administradora_id', $userModel->administradora_id) == $empresa->id ? 'selected' : '' }}>{{ $empresa->nome }}</option>
                                    @endforeach
                                </select>
                                <button type="button" onclick="document.getElementById('empresa-select-field').style.display='none'; document.getElementById('empresa-cnpj-field').style.display='block'; document.getElementById('empresa_cnpj').value='';" class="mt-2 text-sm text-blue-600 hover:text-blue-800">Buscar por CNPJ</button>
                            ...
                            <div id="condominio-field">
                                ...
                                <select id="condominio_id" name="condominio_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach($condominios as $condominio)
                                        <option value="{{ $condominio->id }}" {{ old('condominio_id', $userModel->condominio_id) == $condominio->id ? 'selected' : '' }}>{{ $condominio->nome }}</option>
                                    @endforeach
                                </select>
